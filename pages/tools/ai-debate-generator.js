// File: pages/tools/ai-debate-generator.js

import { useState } from 'react';
import ProtectedRoute from '@/components/ProtectedRoute';

export default function AIDebateGenerator() {
  const [topic, setTopic] = useState('');
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState('');
  const [videoUrl, setVideoUrl] = useState(null);

  const handleGenerate = async () => {
    if (!topic.trim()) return;
    setLoading(true);
    setStatus('Generating AI debate video...');
    setVideoUrl(null);

    // Simulated API delay
    setTimeout(() => {
      setVideoUrl('https://www.w3schools.com/html/mov_bbb.mp4');
      setStatus('Debate video is ready!');
      setLoading(false);
    }, 3000);
  };

  return (
    <ProtectedRoute>
      <div className="max-w-2xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-4">🧠 AI Debate Generator</h1>
        <p className="text-gray-600 dark:text-gray-300 mb-4">
          Enter a topic and watch two AI avatars debate both sides.
        </p>

        <input
          type="text"
          placeholder="e.g. Should social media be banned for kids?"
          value={topic}
          onChange={(e) => setTopic(e.target.value)}
          className="w-full p-2 rounded border dark:bg-zinc-800 dark:border-zinc-600"
        />

        <button
          onClick={handleGenerate}
          disabled={!topic || loading}
          className="mt-4 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50"
        >
          {loading ? 'Generating...' : 'Generate Debate'}
        </button>

        {status && <p className="mt-4 text-sm text-gray-700 dark:text-gray-300">{status}</p>}

        {videoUrl && (
          <video
            src={videoUrl}
            controls
            className="mt-6 rounded-lg shadow-lg w-full aspect-video"
          />
        )}
      </div>
    </ProtectedRoute>
  );
}
